...
module.exports = function historyPlugin(schema, options) {
  ...
  schema.pre('save', function(next) {
    ...
    saveHistoryModel(original, d, historyDoc, this.collection.name, next, session);
  });
  ...
  function saveHistoryModel(original, d, historyDoc, collectionName, next, session = null) {
    const opts = session ? {session} : next;
